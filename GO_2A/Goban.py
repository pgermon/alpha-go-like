...
class Board:
    ...
    def _is_super_ko(self, fcoord, color):
        ...
        positionHash = self._getPositionHash(fcoord, color)
        tmpHash = self._currentHash ^ positionHash
        #print(positionHash)
        assert self._currentHash == tmpHash ^ positionHash
        i = self._neighborsEntries[fcoord]
        libertiesOpponents = {}
        opponent = Board.flip(color)
        while self._neighbors[i] != -1:
